...
# ----------------------------
API_BASE_URL = os.environ.get("QUBLITZ_API_BASE_URL", "").strip()
...
def _try_backend_login(api_key: str) -> Dict[str, Any]:
    if not API_BASE_URL:
        raise RuntimeError("QUBLITZ_API_BASE_URL is not set. For local runs, set it to your SSH tunnel URL.")
    r = requests.get(f"{API_BASE_URL.rstrip('/')}/me", headers={"X-API-Key": api_key}, timeout=8)
    r.raise_for_status()
    return r.json()
...
def page():
    ...
    with tab_freq:
        ...
        start_freq = st.number_input(r"Start $\omega_d/2\pi$ [GHz]", value=fd_start_default, step=0.01, format="%.6f")
        stop_freq = st.number_input(r"Stop $\omega_d/2\pi$ [GHz]", value=fd_stop_default, step=0.01, format="%.6f")
        num_points = st.number_input("Number of frequencies", value=41, min_value=5, max_value=201, step=2)

        spec_tfinal = st.number_input("Pulse duration [ns]", value=25.0, min_value=1.0, max_value=500.0, step=1.0)
        ...
        if out is not None:
            ...
            st.download_button("Download sweep summary CSV", to_csv_bytes(df), "frequency_sweep_summary.csv", "text/csv")

    # ---- Time domain
    with tab_time:
        omega_d_default = float(omega_q) if is_debug_user else 5.0
        omega_d = st.number_input(r'$\omega_d/2\pi$ [GHz]', value=omega_d_default, step=1e-6, format="%.9f")

        t_final = st.number_input(r"Duration $\Delta t$ [ns]", value=200.0, min_value=1.0, max_value=2000.0, step=10.0)
