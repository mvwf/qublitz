...
# ----------------------------
# IMPORTANT:
# - When deployed: set this to your assignment service URL (reachable via VPN if needed).
# - When running locally (developer): default to localhost service.
# API_BASE_URL = os.environ.get("QUBLITZ_API_BASE_URL", "http://127.0.0.1:8001").strip()

API_BASE_URL = os.environ.get(
    "QUBLITZ_API_BASE_URL",
    "http://10.135.171.141:8001"
)
...
def _clip_env(env: np.ndarray) -> np.ndarray:
    # Your simulator expects normalized envelopes. Keep this unless you change the simulator.
...
#         raise RuntimeError(
#             f"Cannot reach assignment service at {base}. "
#             f"Check VPN, URL, and that the service is running. "
#             f"Underlying error: {e}"
#         )

#     # Nice errors for students/instructors
#     if r.status_code == 401:
#         raise RuntimeError("Missing API key header (unexpected).")
...
#     for k in ("omega_q", "omega_rabi", "T1"):
#         if k not in data:
#             raise RuntimeError(f"Server response missing {k!r}. Got keys: {list(data.keys())}")
#     return data
...
def pulse_ui(t_final_ns: float):
    ...
    with col2:
        # Do not advertise clipping to students. Keep UI clean.
